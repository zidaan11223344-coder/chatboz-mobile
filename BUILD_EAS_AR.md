...
يعتمد التطبيق على حساب الأدمن الموجود مسبقاً في قاعدة البيانات ومتغيرات بيئة الخادم. لا تضف `LOCAL_ADMIN_USERNAME` أو `LOCAL_ADMIN_PASSWORD` ولا تستبدل الحساب الموجود. إذا كان البناء يشمل الخادم وقاعدة البيانات، أضف `DATABASE_URL` و`JWT_SECRET` وقيم الخدمات الاختيارية المطلوبة إلى بيئة الخادم أو إعدادات EAS، ولا تضع الأسرار داخل Git.
...
إذا أردت إدخال مفتاح الوصول مباشرة مثل تجربة البناء داخل المحادثة، شغّل السكربت التالي. سيطلب المفتاح بشكل مخفي، ويضعه في الذاكرة أثناء العملية فقط، ثم يرسل بناء APK إلى EAS:

```bash
./build-apk-with-key.sh
```

المفتاح المقصود هو `EXPO_TOKEN` من حساب Expo/EAS. لا تضعه داخل ملفات المشروع ولا ترسله في Git.
...
اختبارات الصلاحيات المحلية تحتاج إلى الاتصال بقاعدة البيانات الحالية وإعدادات الخادم؛ لذلك قد تفشل في بيئة لا تحتوي على `DATABASE_URL` أو لا تصل إلى قاعدة البيانات، وهذا لا يعني أن فحص TypeScript فشل.
