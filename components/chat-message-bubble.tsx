...
export type RealChatMessage = { id: string; senderId: number; senderName?: string | null; kind: "text" | "image" | "audio"; body: string | null; textColor: string | null; attachmentUrl: string | null; durationSeconds: number | null; createdAt: string | Date };
  ...
  return (
    <Animated.View style={[styles.row, mine ? styles.mineRow : styles.otherRow, { transform: [{ translateY }, { translateX }], opacity }]}>
      ...
      <Animated.View style={[styles.bubble, mine ? styles.mineBubble : styles.otherBubble, message.kind === "image" && styles.imageBubble]}>
        ...
        {message.kind === "audio" && (
          <Pressable onPress={() => player.play()} style={styles.audioRow}>
            <MaterialIcons name={status?.playing ? "pause" : "play-arrow"} size={18} color={mine ? "#FFFFFF" : buzzColors.indigo} />
...
const styles = StyleSheet.create({
  ...
  senderName: { fontSize: 11, color: "#5A5A79", marginBottom: 2, writingDirection: "rtl", fontWeight: "700" },
